...
public class ExcelReader {
	...
	// returning the Cell Data form sheet
	 ...
	 * String data) {
	 * 
	 * try { fis = new FileInputStream(path); workbook = new XSSFWorkbook(fis);
	 * 
	 * if (rowNum <= 0) return false; int index = workbook.getSheetIndex(sheetName);
	 * int col_Num = -1; if (index == -1) return false;
	 * 
	 ...
	 * i; } if (col_Num == -1) return false;
	 * 
	 * sheet.autoSizeColumn(col_Num); row = sheet.getRow(rowNum - 1);
	 * 
	 * if (row == null) row = sheet.createRow(rowNum - 1);
	 * 
	 * cell = row.getCell(col_Num); if (cell == null) cell =
	 ...
	 * 
	 * //cell style for links CellStyle link_style=workbook.createCellStyle();
	 * XSSFFont link_font=workbook.createFont();
	 * link_font.setUnderline(XSSFFont.U_SINGLE);
	 * link_font.setColor(IndexedColors.BLUE.getIndex());
	 * link_style.setFont(link_font);
	 * 
	 * XSSFHyperlink link=createHelper.createHyperlink(HyperlinkType.FILE);
	 * link.setAddress(url); cell.setHyperlink(link); cell.setCellStyle(link_style);
	 * 
	 * 
	 * fos = new FileOutputStream(path);
	 * 
